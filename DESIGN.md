...
### Project Brand Tokens (from globals.css)

The running project's custom CSS variables in `globals.css` define a complementary brand palette used alongside the Apple-style system colors:

- **Brand Primary** (`{colors.brand-primary}` — #121212): Near-black. Used as the dominant surface/ink color in the custom theme — nav bars, primary text, and key UI chrome.
- **Brand Silver** (`{colors.brand-silver}` — #C8CDD2): Cool light gray. Used for subtle metallic accents, secondary text on dark surfaces, and muted dividers.
- **Brand Red** (`{colors.brand-red}` — #D82E2F): The functional accent color for sale prices, badges, errors, and high-attention CTAs.
- **Brand Red Hover** (`{colors.brand-red-hover}` — #E83E3F): A slightly brighter red for interactive hover/focus states on red elements.
- **Brand Dark** (`{colors.brand-dark}` — #333333): Charcoal gray for secondary surfaces, footer backgrounds, and softer dark UI regions.
- **Brand Light** (`{colors.brand-light}` — #F5F5F5): Light neutral canvas. Page background, card surfaces, and light section fills.
- **Brand White** (`{colors.brand-white}` — #FFFFFF): Pure white for text on dark surfaces, button fills, and maximum-contrast moments.
